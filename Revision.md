...
1. 44.23 (Docker command)
2. 1.16.26 (Docker Network)
3. Dockerfile acts as a blue print for building images
- Start by basing it on another image (FROM node)
- ENV sets environment variables
- RUN runs commands (like installing packages) (Like apt-get install)
- COPY copies files from host to image
- CMD specifies the command to run when a container is started from the image (executes when container starts basically entry point)


To build an image from Dockerfile : docker build -t image_name:tag . (the dot means current directory where Dockerfile is present) we will get image id after building the image

--------------------------------------------------------------------

Docker Volumes :

these are used to persist data generated by and used by Docker containers.

Actually when we create a container from an image, all the data inside the container is ephemeral, means if we delete the container all the data inside the container is lost.

3 ways to create volumes :

1. Using docker CLI : docker volume create volume_name
                                             local_volume_path                      :/container_path_inside_docker_container
2. Using docker run command : docker run -v /home/AbhayGo/66_DockerInRealtime/images:/app/images image_name
3. Using docker compose file : 
   volumes:
     volume_name:
       driver: local    

4. Where Docker stores volumes on host machine ?
   Docker stores volumes in a specific directory on the host machine, which varies depending on the operating system being used.
    - On Linux, Docker stores volumes in the /var/lib/docker/volumes/ directory